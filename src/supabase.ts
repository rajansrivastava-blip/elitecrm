...
async function selectAllFromTable(supabaseClient: any, tableName: string, columns: string = "*"): Promise<any[]> {
  ...
  while (true) {
    ...
    console.log(`[selectAllFromTable] Table "${tableName}": Fetching range ${from} to ${to} (page ${page})`);
    ...
    if (error) {
      console.error(`[selectAllFromTable] Table "${tableName}" error on page ${page}:`, error);
      throw error;
    }
    if (!data || data.length === 0) {
      console.log(`[selectAllFromTable] Table "${tableName}": Reached end of table at page ${page} (0 rows returned)`);
      break;
    }
    allRows.push(...data);
    console.log(`[selectAllFromTable] Table "${tableName}": Page ${page} returned ${data.length} rows. Cumulative count: ${allRows.length}`);
    if (data.length < pageSize) {
      break;
    }
    page++;
  }
  console.log(`[selectAllFromTable] Table "${tableName}": Fully loaded ${allRows.length} total rows.`);
  return allRows;
}
